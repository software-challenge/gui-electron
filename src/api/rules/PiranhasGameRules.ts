import { ALL_DIRECTIONS, Board, Coordinates, Direction, FIELDSIZE, FIELDTYPE, LineDirection, Move, Player, PLAYERCOLOR } from './Piranhas'
...
export class GameRuleLogic {
  ...
  static addBlockedFields(board: Board) {
    const numberOfBlockedFields = 2
    const start = 2 // first row or column, in which blocked fields are allowed
    const end = 7 // last row or column, in which blocked fields are allowed

    // create a list of coordinates for fields which may be blocked
    let blockableFieldCoordinates = Array.from({length: end - start + 1}, (_, k) => {
      const x = k + start
    ...
    for (var i = 0; i < numberOfBlockedFields; i++) {
      ...
      const coordinatesToRemove = ALL_DIRECTIONS.map(
  ...
  static possibleMoves(board: Board, field: Coordinates): Move[] {
    const fishColorToMove = board.field(field)
