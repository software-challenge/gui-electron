...
import { Scene, ThreeDObject, Texture, ThreeDEntity } from './FileDefs';
...
export class Engine {
  ...
  constructor(canvas: HTMLCanvasElement, assetPath: string = 'scene.bin') {
    ...
    for (var o in scene.objects) {
      ...
      var so: ThreeDObject = scene.objects[o];
      var vertexShader: WebGLShader = this.context.createShader(this.context.VERTEX_SHADER);
      var fragmentShader: WebGLShader = this.context.createShader(this.context.FRAGMENT_SHADER);
    ...
    for (var t in scene.textures) {
      var texture: WebGLTexture = this.context.createTexture();
      var st: Texture = scene.textures[t];
  ...
  drawBrush(name: string, uniforms: { [name: string]: { value: number[], type: UniformType } }) {
    ...
    for (var i = 0; i < brush.textures.length; i++) {
