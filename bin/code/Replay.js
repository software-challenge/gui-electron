define(["require", "exports"], function (require, exports) {
    ...
    class Replay {
        ...
        findPassengers() {
            ...
            var hasPassengerAt = (x, y) => this.passengers.findIndex((passenger => passenger.x == x && passenger.y == y)) != -1;
            var isPassengerFieldType = (ft) => ft == 3 /* PASSENGER0 */ || ft == 4 /* PASSENGER1 */ || ft == 5 /* PASSENGER2 */ || ft == 6 /* PASSENGER3 */ || ft == 7 /* PASSENGER4 */ || ft == 8 /* PASSENGER5 */ || ft == 9 /* PASSENGER6 */;
            var next_passenger_id = 0;
            for (var i = 0; i < this.states.length; i++) {
                var tiles = this.states[i].board.tiles;
                //1. Find new passengers
                tiles.forEach(tile => {
                    if (processedTileIds.indexOf(tile.index) == -1) {
                        tile.fields.forEach(field => {
                            if (isPassengerFieldType(field.type) && !hasPassengerAt(field.x, field.y)) {
                                var p = new Passenger();
                ...
                this.passengers.forEach(passenger => {
                    if (this.states[i].board.tileIndices.indexOf(passenger.tile_id) != -1) {
                        if (!isPassengerFieldType(this.states[i].board.getTileByIndex(passenger.tile_id).getFieldByIndex(passenger.x, passenger.y).type)) {
     ...
     */
    class Score {
        constructor(resultNode, redName, blueName) {
            ...
            this.loserName = this.winner == 0 /* RED */ ? blueName : redName;
